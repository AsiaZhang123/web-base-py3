...
from werkzeug.routing import Rule, Map, MapAdapter, RequestPath, RequestRedirect, RequestAliasRedirect, _simple_rule_re
...
class MFFlask(Flask):
    ...
    default_config = ImmutableDict({
        ...
        'SECRET_KEY': None,  # 密匙 使用flask自带session时，加密session时使用。
        ...
        # 如果被设置为 True （这是默认值），每一个请求 cookie 都会被刷新。如果设置为 False ，只有当 cookie 被修改后才会发送一个 set-cookie 的标头
        ...
        'JSON_AS_ASCII': True,
        # Flask 按照 JSON 对象的键的顺序来序来序列化它。这样做是为了确保键的顺序不会受到字典的哈希种子的影响，从而返回的值每次都是一致的，不会造成无用的额外 HTTP 缓存
        'JSON_SORT_KEYS': True,
        # 如果这个配置项被 True （默认值）， 如果不是 XMLHttpRequest 请求的话（由 X-Requested-With 标头控制） json 字符串的返回值会被漂亮地打印出来。
        'JSONIFY_PRETTYPRINT_REGULAR': False,
        'JSONIFY_MIMETYPE': 'application/json',
        'TEMPLATES_AUTO_RELOAD': None,
        # SameSite限制如何使用来自外部站点的请求发送cookie。可以设置为“lax”（推荐）或“strict”。LAX防止从外部站点（如提交表单）发送带有易受CSRF影响的请求的cookie。strict禁止发送带有所有外部请求的cookie，包括以下常规链接。
